#!/bin/bash
# 02-recover-and-validate.sh
# Met à jour vm-agent avec la version corrigée et relance la validation M1
...
echo "── 1. MISE À JOUR VM-AGENT BINARY ──"
cp "$REPO_DIR/scripts/vm-agent.sh" /usr/local/bin/vm-agent
chmod +x /usr/local/bin/vm-agent
echo "✓ /usr/local/bin/vm-agent mis à jour"

echo ""
echo "── 2. DÉPLOIEMENT SEED.JS ──"
